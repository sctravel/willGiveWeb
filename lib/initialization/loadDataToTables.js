...
var loadDataFromTxtToTable = function( tableColumnNames, tableColumnTypes, filePath, delimiter, tableName) {
   ...
   " tableColumnTypes is "+ tableColumnTypes.length+". They should be equal!");
    ...
    var query = "insert into users (email, password, firstname, lastname) values ('admin@admin.com', '" + encryptionUtil.saltAndHash('admin@admin.com') + "', 'Super', 'Admin')"; 
	console.log(query);
	connection.query(query
	  ,function(err,results){if (err != null) console.error(err);}
	); 
