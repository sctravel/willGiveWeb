...
function getQRCodeContentForRecipient( recipient ) {
	...
	if (mission.length > 203) mission = mission.substring(0,200) + " ...";
    var message = 'http://www.willgive.org/c/'+recipient.ein.trim()+'?r=' + recipient.recipientId +'&n='+name+'&a='+address+'&p='+recipient.phone.trim()+'&m='+mission;
    return message;
}