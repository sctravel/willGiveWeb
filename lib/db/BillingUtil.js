...
exports.updatePaymentMethodStripeId = function(userId,stripe_customer_id,last4, callback) {
    ...
    /*
        ...
        if(results[0] == undefined){
            ...
            var sqlInsert = ' insert into ' + tableNames.PAYMENT_METHOD_TABLE + " ( userId, stripeCustomerId,type,cvc,address,phone,nameOnCard,expirationDate,last4) values " +
            ...
            console.dir("last4:"+ last4);

            dbPool.runQueryWithParams(sqlInsert, [userId,stripe_customer_id,last4], function (err, results) {
                if(err) {
                    console.error(err);
                    callback(err,null);
                    return;
            ...
            var sqlupdate = ' update ' + tableNames.PAYMENT_METHOD_TABLE + ' set stripeCustomerId = ?, last4 =? where userId = ? ';
            console.dir(sqlupdate);


            dbPool.runQueryWithParams(sqlupdate, [stripe_customer_id,last4, userId,], function (err, results) {
                if(err) {
                    console.error(err);
                    callback(err,null);
                    return;
...
var queryExistingStripCustomers = function(userId, callback) {
    ...
    var sql = 'select stripeCustomerId, last4 from ' + tableNames.PAYMENT_METHOD_TABLE + ' where userId=? ';
