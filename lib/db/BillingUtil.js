...
exports.insertTransactionHistroy = function(transaction_id,amount,user_id,recipient_id, status,stripeToken, callback) {
    var sql="insert into  " + tableNames.TRANSACTION_TABLE + " ( transaction_id, amount,user_id,recipient_id,dateTime,settleTime,status,stripeToken,source,confirmationCode) values (?, ?, ?, ?, NOW(), NOW(), ?, ?,'Web',UUID()) ";
...
exports.getTransactionHistoryBasedOnConfirmationCode = function(confirmationCode, callback) {
    var sql = 'select transaction_id, amount, user_id, recipient_id,dateTime,notes from ' + tableNames.TRANSACTION_TABLE +
        ' where  confirmationCode = ? order by datetime desc ';
