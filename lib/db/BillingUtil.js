...
var updateTransactionHistroy = function (confirmationCode, callback) {
    ...
    var sqlupdate = ' update ' + tableNames.TRANSACTION_TABLE + ' set status = \'Processed\' where confirmationCode = ? ';
    console.dir(sqlupdate);


    dbPool.runQueryWithParams(sqlupdate, [confirmationCode], function (err, results) {
        if(err) {
            console.error(err);
            callback(err,null);
            return;
...
exports.insertUserPledge = function(userPledge, callback) {
    var sql = "insert into " + tableNames.TRANSACTION_TABLE + "( transactionId, amount, userId, recipientId, status, confirmationCode ) values ( ?, ?, ?, ?, ?, UUID() ) ";
...
exports.getTransactionHistoryBasedOnConfirmationCode = function(confirmationCode, callback) {
    var sql = 'select transactionId, amount, u.first_name, u.last_name, recipientId,dateTime,notes from ' + tableNames.TRANSACTION_TABLE + ' t ,' + tableNames.USER_TABLE + 'u' +
        ' where t.user_id= u.user_id and confirmationCode = ? and status= \'Processed\' order by datetime desc ';
