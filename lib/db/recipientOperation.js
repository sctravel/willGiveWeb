...
exports.createNewRecipient = function(recipient, callback) {
    ...
    var insertSql = 'Insert into ' + tableNames.RECIPIENT_TABLE +
        ...
        '   ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ' +
        '   ?, ?, ?, ?, ?  ) ' ;
    var params = [recipient.email.trim(), encryptionUtil.saltAndHash(recipient.password), recipient.organizationName.trim(),
...
exports.getRecipientAccountInfo = function(recipientId, callback) {
    ...
    var sql = 'select name, EIN, status, category, address, city, state, zipcode, country, phone, fax, ' +
        '   contact_person_name, contact_person_title, mission, website, facebook_url from ' + tableNames.RECIPIENT_TABLE + ' where recipient_id = ? ';
    var recipient = {};
    ...
    dbPool.runQueryWithParams(sql,[recipientId],function(err,results){
        ...
        if(results && results.length==1) {
            ...
            recipient.recipiendId = recipientId;
            recipient.name = results[0].name;
            recipient.EIN = results[0].EIN;
            recipient.category = results[0].category;
            recipient.status = results[0].status;
            recipient.address =results[0].address;
            recipient.city = results[0].city;
            recipient.state = results[0].state;
            recipient.zipcode = results[0].zipcode;
            recipient.country = results[0].country;
            recipient.phone = results[0].phone;
            recipient.fax = results[0].fax;
            recipient.contactPersonName = results[0].contact_person_name;
            recipient.contactPersonTitle = results[0].contact_person_title;
            recipient.mission = results[0].mission.toString('utf-8');
            logger.warn('!!!!!!mission!!!!!!!');
            logger.warn(recipient.mission);
            recipient.website = results[0].website;
            recipient.facebookUrl = results[0].facebook_url;
