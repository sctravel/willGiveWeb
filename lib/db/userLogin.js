...
function loginUserLoginHistory(userId, callback) {
    ...
    dbPool.runQueryWithParams(sql,[userId, sessionId],function(err,results) {
        ...
        if(err) {
            logger.error(err + ".\n userId-"+userId+" logged in failed in loginUserLoginHistory.");
            callback(err, null);
            return;
        }
        ...
        if(results && results.affectedRows==1) {
            ...
            logger.error("userId-"+userId+" logged in failed in loginUserLoginHistory. Query result is not expected.");
...
exports.logoutUserLoginHistory = function(userId, sessionId, callback) {
    ...
    dbPool.runQueryWithParams(sql,[userId, sessionId],function(err,results) {
        if(err) {
            logger.error(err+"\nuserId-"+userId+" logged out failed in logoutUserLoginHistory.");
            callback(err, null);
            return;
        }
        console.dir(results);
        if(results&&results.affectedRows==1) {
            ...
            logger.error("userId-"+userId+" logged out failed in logoutUserLoginHistory. Query result is not expected.");
...
exports.manualLogin = function(email, pass, callback) {
    ...
    logger.info("user -"+email+"with " + constants.login.LOGIN_PROVIDER.WILLGIVE +" logging in.");

    //TODO hash the password
    conn.runQueryWithParams(authSql,[email, constants.login.LOGIN_PROVIDER.WILLGIVE], function(err,results) {
        if(err) {
            logger.error(err+"\email-"+email+" logged in failed. Email or password is incorrect. Please try it again. ");
        ...
        if(results && results.length==1) {
            if(encryptionUtil.validatePassword(pass,results[0].password )) {
                ...
                loginUserLoginHistory(results[0].userId, function(err, logResult){
                    ...
                    logger.info("user -"+email+"with " + constants.login.LOGIN_PROVIDER.WILLGIVE +" logged in successfully.");
            ...
            logger.error("email-"+email+" logged in failed. Email doesn't exist. Please try it again. ");
...
function verifyUser(userId, sessionId, callback) {
    ...
    dbPool.runQueryWithParams(verifySql, [userId], function(err, results){
        if(err) {
            logger.error(err+"\nVerify user with id -"+userId+" SQL failed.");
            callback(err,null);
            return;
        }
        if(results && results.length>=1) {
            ...
            logger.error("Verify user with id -"+userId+" failed. no SessionId matches");
            callback(new Error("no SessionId matches"),null);

        } else {
            logger.error("Verify user with id -"+userId+" failed. Result error");
...
exports.loginOrCreateAccountWithFacebook = function(fbUserData,callback) {
    ...
    var returnObj={};
    logger.info("user -"+userInfo.email+"with " + userInfo.provider +" logging in.");
    ...
    dbPool.runQueryWithParams(sql,[userInfo.email, userInfo.provider],function(err,results){
        ...
        if(results && results.length==1) { //the account is already registered
            ...
            addNewUserAccount(userInfo,function(err,results){
                if(err) {
                    logger.error(err + ". AddNewUserAccount - Please enter your first and last name.");
                ...
                loginUserLoginHistory(userId, function(err, logResult){
                    ...
                    returnObj.email = userInfo.email;
                    returnObj.sessionId = logResult.sessionId;
                    logger.info("user -"+userInfo.email+"with " + userInfo.provider +" logged in successfully.");
                    callback(null, returnObj);
                })
...
exports.getUserTransactionHistory = function(userId, callback) {
    var sql = 'select ut.transactionId, ut.confirmationCode, ut.amount, ut.recipientId, r.name,  r.imageUrl, ' +
        ...
        ' ut.userId = ? and ut.status = \'Paid\' order by ut.settleTime desc ';
