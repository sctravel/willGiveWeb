...
exports.manualLogin = function(email, pass, callback) {
    ...
    conn.runQueryWithParams(authSql,[email, constants.login.LOGIN_PROVIDER.WILLGIVE], function(err,results) {
        if(err) {
            returnObj.errorMessage="Login failed. Please try it again";
        ...
        if(results && results.length==1) {
            if(encryptionUtil.validatePassword(pass,results[0].password )) {
                ...
                loginCustomerLoginHistory(results[0].user_id, function(err, logResult){
                    if(err) {
                        console.err(err);
                        callback(err,returnObj);
                        return;
                    ...
                    returnObj.customerId = results[0].user_id;
