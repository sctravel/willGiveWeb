...
var markLinkInActive = function(email,randomString, callback) {
    var markSql = "update "+tableNames.USER_FIND_PASSWORD + " set is_active='N' where email= ? and random_string = ? and is_active='Y' ";
...
exports.forgotPassword = function(email,callback){
    ...
    dbPool.runQueryWithParams(checkEmailSql,[email],function(err,results){
        ...
        if(results && results.length==1) {
            var sql = "insert into "+tableNames.USER_FIND_PASSWORD + " (email,random_string) values (?,?) ";
