...
function getTransactionsByCharityId( charityId, callback ) {
    var sql = ' select ut.userId, users.firstName, users.lastName, ut.amount, ' +
        ' ut.datetime, ut.status, users.imageIconUrl, us.transactionPublic from user_transactions ut inner join users on ' +
        ' ut.userId = users.userId  join user_settings us on users.userId = us.userId where ut.recipientId = ? and ut.status= ? '
        ;
