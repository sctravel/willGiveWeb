...
exports.logQRScanHistory = function(userId, recipientId, latitude, longitude) {
    var sql = 'insert into ' + tableNames.QR_SCAN_HISTORY + ' ( userId, recipientId, latitude, longitude ) Values ( ?, ?, ?, ? ) ';

    dbPool.runQueryWithParams(sql,[userId, recipientId, latitude, longitude],function(err,results) {
        if(err) {
            logger.error(err);
            callback(err,null);
            return;
...
function getTransactionsByCharityId( charityId, callback ) {
    var sql = ' select ut.userId, users.firstName, users.lastName, ut.amount, ' +
        ' ut.datetime, ut.settleTime, ut.status, users.imageIconUrl, us.transactionPublic from ' +
...
function getTransactionSummaryByCharityId( charityId, duration, callback ) {
    var sql = ' select sum(ut.amount), count(1), r.recipientId from ' + tableNames.TRANSACTION_TABLE +
        ...
        ;
    if(duration !=null ) {
        sql += ' and settleDatetime BETWEEN DATE_SUB(NOW(), INTERVAL ? DAY) AND NOW() ';
        dbPool.runQueryWithParams( sql, [charityId, constants.paymentStatus.PAID, duration], function(err, results){
            if(err) {
                logger.error(err);
                callback(err,null);
                return;
...
exports.getTransactionSummaryByCharityId = getTransactionSummaryByCharityId;
