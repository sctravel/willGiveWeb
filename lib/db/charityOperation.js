...
function listAllCharity( userId, start, count, callback ) {
    ...
    if (userId != null)
    {
        sql = ' select recipientId, category, status, email, name, ein, address, zipCode, city, state, country, phone, fax, website, ' +
            ' CAST(mission AS CHAR(10000) CHARACTER SET utf8) as mission, status, facebookUrl, ' +
            ' videoUrl, imageUrl, ' +
            ' contactPersonName, contactPersonTitle, ' +
            ' rating, ifNull( isEnabled, "N") as isFavored from ' + tableNames.RECIPIENT_TABLE +
...
function charityByEIN(ein, callback){
    ...
    dbPool.runQueryWithParams(sql,ein,function(err,results) {
        ...
        if(results && results.length==1) {
            ...
            logger.error('No result or more than 1 result is found - ' + results.length);
...
function getTransactionsByCharityId( charityId, callback ) {
    var sql = " select ut.transactionId, ut.userId, users.firstName, users.lastName, ut.amount, " +
        " ut.datetime, ut.status, users.email, users.imageIconUrl from user_transactions ut inner join users on " +
        " ut.userId = users.userId  where ut.recipientId = ? ";
